...
DrawCard()
{
    global GuiHwnd, GuiW, GuiH, ColFill, ColStroke, ColOrange, ColLav, ColBlue, ColInk, word, cnt, riseTxt, setTxt, sunT, BgFade
    w := GuiW, h := GuiH, r := 44.0
    ...
    if (BgFade > 0.02)
    {
        ...
        if (sa < 1)
            sa := 1
        DllCall("gdiplus\GdipCreatePen1", "UInt", GlowColor(ColStroke, sa), "Float", 1.4, "Int", 2, "Ptr*", pEdge)
        DllCall("gdiplus\GdipDrawPath", "Ptr", G, "Ptr", pEdge, "Ptr", pPath)
        DllCall("gdiplus\GdipDeletePen", "Ptr", pEdge)
    }
    DllCall("gdiplus\GdipDeletePath", "Ptr", pPath)

    DrawSunIcon(G, 28.0, 24.0, ColOrange)
    DrawMoonIcon(G, w - 28.0, 24.0, ColBlue)
    ...
    a0 := 232.0
    a1 := 308.0
    DrawSunArc(G, cx, cy, rArc, a0, a1, 8.0, 0.16)
    DrawSunArc(G, cx, cy, rArc, a0, a1, 3.2, 1.0)
    ...
    DllCall("gdiplus\GdipFillEllipse", "Ptr", G, "Ptr", bGlow, "Float", sx - 9, "Float", sy - 9, "Float", 18, "Float", 18)
    DllCall("gdiplus\GdipDeleteBrush", "Ptr", bGlow)
    DllCall("gdiplus\GdipCreateSolidFill", "UInt", colDot, "Ptr*", bDot)
    DllCall("gdiplus\GdipFillEllipse", "Ptr", G, "Ptr", bDot, "Float", sx - 4.2, "Float", sy - 4.2, "Float", 8.4, "Float", 8.4)
    DllCall("gdiplus\GdipDeleteBrush", "Ptr", bDot)

    DrawText(G, cnt, 0, 62, w, 42, 26, ColInk, 1, 1)
    DrawText(G, riseTxt, 22, 118, 120, 36, 24, ColOrange, 0, 1)
    DrawText(G, setTxt, w - 142, 118, 120, 36, 24, ColBlue, 2, 1)
...
DrawSunIcon(G, cx, cy, col)
{
    ...
    DllCall("gdiplus\GdipFillEllipse", "Ptr", G, "Ptr", bGlow, "Float", cx - 11, "Float", cy - 11, "Float", 22, "Float", 22)
    DllCall("gdiplus\GdipDeleteBrush", "Ptr", bGlow)
    DllCall("gdiplus\GdipCreateSolidFill", "UInt", col, "Ptr*", b)
    DllCall("gdiplus\GdipFillEllipse", "Ptr", G, "Ptr", b, "Float", cx - 6.2, "Float", cy - 6.2, "Float", 12.4, "Float", 12.4)
    DllCall("gdiplus\GdipDeleteBrush", "Ptr", b)
    DllCall("gdiplus\GdipCreatePen1", "UInt", col, "Float", 1.8, "Int", 2, "Ptr*", pPen)
    ...
    Loop, 8
    {
        ...
        x0 := cx + 8.2 * Cos(ang), y0 := cy + 8.2 * Sin(ang)
        x1 := cx + 12.4 * Cos(ang), y1 := cy + 12.4 * Sin(ang)
...
DrawMoonIcon(G, cx, cy, col)
{
    ...
    y := cy - d / 2.0
    DllCall("gdiplus\GdipCreateSolidFill", "UInt", GlowColor(col, 0x50), "Ptr*", bGlow)
    DllCall("gdiplus\GdipFillEllipse", "Ptr", G, "Ptr", bGlow, "Float", cx - 12, "Float", cy - 12, "Float", 24, "Float", 24)
    DllCall("gdiplus\GdipDeleteBrush", "Ptr", bGlow)
    ...
    DllCall("gdiplus\GdipAddPathEllipse", "Ptr", pCut, "Float", x - 5.0, "Float", y - 1.2, "Float", d, "Float", d)
